...
with st.sidebar:
    ...
    for step in ["① Flight Agent", "② Hotel Agent", "③ Itinerary Agent", "④ Final Agent", "⑤ Email Agent"]:
...
st.markdown("<div class='input-label'>📧 Recipient Email Address</div>", unsafe_allow_html=True)
recipient_email = st.text_input(
    "", 
    placeholder="Enter email to automatically forward final itinerary email (e.g. customer@example.com)",
    label_visibility="collapsed"
).strip()
...
AGENT_META = {
    ...
    "email_agent":     ("📬", "Email Dispatch Agent")
}
...
if generate:
    if not user_query.strip():
            ...
            for node_name, state_update in chunk.items():
                ...
                with st.status(f"{icon}  {label}", state="complete", expanded=True):
                    if node_name == "flight_agent":
                        ...
                        text = state_update.get("itinerary", "")
                        if not text and state_update.get("messages"):
                            text = state_update["messages"][-1].content
                        collected["final_response"] = text
                        st.markdown(text or "_No final response generated._")

                    elif node_name == "email_agent":
                        msgs = state_update.get("messages", [])
                        text = msgs[-1].content if msgs else "Email processing handled."
                        collected["email_status"] = text
        ...
        if collected['email_status']:
            st.markdown("<br>", unsafe_allow_html=True)
            st.info(f"📧 **Automation Alert:** {collected['email_status']}")
